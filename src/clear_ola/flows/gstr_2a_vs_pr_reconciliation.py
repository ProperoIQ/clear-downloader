"""2A vs PR Reconciliation — PAN-based recon of GSTR-2A vs Purchase Register.

Compares ITC available per GSTR-2A (pulled from GSTN) against the Purchase
Register (PR) the taxpayer uploads into Clear. Clear's UI path is
`/reconciliation/idt/2aVsPr`; the match type is `MAX_ITC_2A_PR`. Output: one
PAN-level XLSX per FY.

This report uses the SAME recon "matching task" backend as the 2B-vs-PR report
(`/api/recon/ultimatum/public/...`), differing only in the matchType. Instead of
the data-pull -> data-browser export pipeline, it drives the recon flow,
confirmed from
discovery/app.clear.in.har__2B vs PR Reconciliation_2A vs PR Recon.har:

  1. POST matching/v2/trigger            -> taskId (kicks off the match; also
                                            fetches the 2A/PR data server-side)
...
Both the PR and 2A sides of the match use the SAME period range (per the HAR).
...
def run(api: ClearAPI, cfg: AppConfig, manifest: Manifest) -> None:
    """Process every (PAN x FY) in the config for the 2A-vs-PR reconciliation.
    ...
    for pan_cfg in cfg.pans:
        ...
        logger.info(
            ...
            "Generating PAN-level 2A-vs-PR reconciliation per FY.",
...
def _run_one(
    *,
    ...
    try:
        ...
        start_rp, end_rp = rng
        logger.info(
            "[{}/{}] period range {}..{} (PR and 2A sides identical)",
        ...
        # Step 1/4: trigger the match (also fetches the 2A/PR data server-side).
        logger.info("[{}/{}] Step 1/4: trigger 2A-vs-PR match", pan, fy)
