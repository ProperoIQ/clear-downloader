"""PAN Cash Ledger Report — one full download per (PAN x FY).

Aggregates the per-GSTIN cash ledger entries across every GSTIN under a PAN
for a given date range. Clear's UI slug is `panCashLedger`. Output: one
PAN-level XLSX per FY (we map each configured FY to its `01-04-YYYY` →
`31-03-YY+1` date range; FY 2017-18 is clamped to `01-07-2017` because
GST began that month).

Structurally closest to `gstr_2a.py` — single pull, single export, no
preflight, no reconciliation cube. Key differences:

  1. Pull tenant + RLS workflow are `CASH_LEDGER_REPORT`.
  2. Referer slug is `reportType=panCashLedger` with `timePeriodType=DATE_RANGE`
     (FYs use `FISCAL_YEAR`).
  3. Date scope is sent as **DD-MM-YYYY** strings (not `MMYYYY` periods).
     `api.trigger_pull` and `api.fetch_rls_token` both have date-range modes
     that pass the strings through verbatim.
  4. Export filename is `PAN_CASH_LEDGER_REPORT_<PAN>_<start>-<end>`.
  5. `gisDownloadBehaviour` is `None` (HAR sent JSON `null`, not
     `"USE_EXISTING_DATA"`).

Captured POST body lives next to this file as
`pan_cash_ledger_statement.json`.
"""
...
def _load_statement_template() -> dict:
    """Load the verbatim export-trigger payload captured for panCashLedger."""
...
def _fy_to_date_range(fy: str, *, as_of: date) -> tuple[str, str]:
    ...
    Rules:
      ...
      - End:   31-03-<second-year> (next March), clamped down to today for
        the current FY (asking the cash ledger for future dates returns
        empty data anyway).
    """
...
def _build_export_payload(
    *,
    ...
    `$TEMPLATE` reference (id `67e2a6e78ede5b3eac89594d`) server-side.
    """
    filename_base = f"PAN_CASH_LEDGER_REPORT_{pan}_{start_range}-{end_range}"
...
def run(
    ...
    """Process every (PAN x FY) in the config for PAN Cash Ledger. Skips combos
    ...
    same partial-handling pattern as every other flow.
    """
    ...
    for pan_cfg in cfg.pans:
        ...
        logger.info(
            ...
            "Generating PAN-level Cash Ledger per FY.",
...
def _run_one(
    *,
    ...
    try:
        ...
        logger.info(
            "[{}/{}] Date range for cash-ledger pull: {} .. {}",
            pan, fy, start_range, end_range,
        )

        # Step 1: Trigger cash-ledger pull
        logger.info(
            "[{}/{}] Step 1/5: refresh Cash Ledger data for {} underlying "
            "GSTINs ({}..{}) under tenant {}",
        ...
        pull_id = api.trigger_pull(
            ...
            tenant=TENANT,
            gis_download_behaviour=None,  # HAR sent JSON null, not the default string
        )
        ...
        logger.info(
            "[{}/{}] Step 2/5: wait for the cash-ledger data refresh", pan, fy,
        )
        ...
        if not_applicable_count > 0:
            logger.info(
                ...
                "the PAN-level cash ledger will only contain data from the "
                "{} GSTIN(s) that did.",
        ...
        if issue_rows:
            ...
            logger.warning(
                "[{}/{}] Cash-ledger pull settled with issues: {}. "
            ...
            logger.info(
                "[{}/{}] Cash-ledger pull settled cleanly. Continuing to export.",
                pan, fy,
            )
        time.sleep(cfg.inter_call_delay_seconds)

        # Per-call header overrides for the panCashLedger export trigger.
        # Verified from HAR (entries 108/118/136): x-ct-source is absent,
        # baggage + sentry-trace are present, accept-language en-US, priority u=1.
        ...
        # Referer must carry reportType=panCashLedger and timePeriodType=DATE_RANGE,
        # or Clear's endpoint may 500 (same edge-validation pattern as the
        # reconciliation endpoints).
        report_referer = (
            "https://app.clear.in/gst/reports/v2"
            f"?reportType=panCashLedger"
        ...
        logger.info(
            "[{}/{}] Step 4/5: trigger PAN-level cash-ledger export", pan, fy,
        )
