"""Outward E-invoice Report — one Excel per (PAN × FY)."""
...
REPORT_TYPE = "Outward-E-Invoice-Report"
TENANT = "GSTR1_EINVOICE"
RLS_WORKFLOW = "GSTR1_EINVOICE"


def _load_statement_template() -> dict:
    """Load the verbatim export-trigger payload captured from the HAR.

    Stored as package data at `clear_ola/flows/outward_e_invoice_report_statement.json`."""
    with resources.files("clear_ola.flows").joinpath(
        "outward_e_invoice_report_statement.json"
...
def _build_export_payload(
    *,
    ...
    """Take the captured Outward E-invoice statement and substitute
    PAN/FY/workspace-specific bits.
    ...
    The `statement` block (fields, filters=null) is left untouched — Clear
    decides scope from headers + workspace context.
    """
    start_range = periods[0]   # e.g. "042025"
    end_range = periods[-1]    # e.g. "032026"
    filename_base = f"PAN_GSTR1_EINVOICE_{pan}_{start_range}-{end_range}"
    ...
    # exportName stays as the captured value ("gstr1_einvoice_download") — it's
    # the S3 prefix Clear uses for this report type.
    ...
    for callback_key in ("onStart", "onFinish"):
        ...
        # md["reportType"] stays "gstr1EinvoiceReport" — that's what makes this
        # the Outward E-invoice Report.
...
def run(
    ...
    """Process every (PAN × FY) in the config for the Outward E-invoice Report.
    Skips combos already marked done. Records progress + errors to the manifest
    after every step.
...
def _run_one(
    *,
    ...
    try:
        ...
        logger.info(
            "[{}/{}] Step 1/6: refresh Outward E-invoice Report data for {} underlying GSTINs "
