"""8A vs PR Reconciliation — PAN-based recon of GSTR-8A vs Purchase Register.

Compares TCS credit reflected per GSTR-8A (pulled from GSTN) against
the Purchase Register (PR) the taxpayer uploads into Clear. Clear's UI path is
`/reconciliation/idt/8aVsPr`; the match type is `MAX_ITC_8A_PR`. Output: one
...
flow, confirmed from
discovery/app.clear.in.GSTR-8A vs PR Recon.har:

  1. POST matching/v2/trigger            -> taskId (kicks off the match; also
                                            fetches the 8A/PR data server-side)
...
Both the PR and 8A sides of the match use the SAME period range (per the HAR).
...
def run(api: ClearAPI, cfg: AppConfig, manifest: Manifest) -> None:
    """Process every (PAN x FY) in the config for the 8A-vs-PR reconciliation.
    ...
    for pan_cfg in cfg.pans:
        ...
        logger.info(
            ...
            "Generating PAN-level 8A-vs-PR reconciliation per FY.",
...
def _run_one(
    *,
    ...
    try:
        ...
        start_rp, end_rp = rng
        logger.info(
            "[{}/{}] period range {}..{} (PR and 8A sides identical)",
        ...
        # Step 1/4: trigger the match (also fetches the 8A/PR data server-side).
        logger.info("[{}/{}] Step 1/4: trigger 8A-vs-PR match", pan, fy)
