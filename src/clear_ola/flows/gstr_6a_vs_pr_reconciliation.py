"""6A vs PR Reconciliation — PAN-based recon of GSTR-6A vs Purchase Register.

Compares ITC available per GSTR-6A (ISD auto-drafted, pulled from GSTN) against
the Purchase Register (PR) the taxpayer uploads into Clear. Clear's UI path is
`/reconciliation/idt/6aVsPr`; the match type is `MAX_ITC_6A_PR`. Output: one
PAN-level XLSX per FY.

This report uses the SAME recon "matching task" backend as the 2A/2B-vs-PR
reports (`/api/recon/ultimatum/public/...`), differing only in the matchType.
Instead of the data-pull -> data-browser export pipeline, it drives the recon
flow, confirmed from
discovery/app.clear.in.har__6A vs PR recon.har:

  1. POST matching/v2/trigger            -> taskId (kicks off the match; also
                                            fetches the 6A/PR data server-side)
...
Both the PR and 6A sides of the match use the SAME period range (per the HAR).
...
REPORT_TYPE = "6A-vs-PR-Reconciliation"
MATCH_TYPE = "MAX_ITC_6A_PR"
...
def run(api: ClearAPI, cfg: AppConfig, manifest: Manifest) -> None:
    """Process every (PAN x FY) in the config for the 6A-vs-PR reconciliation.
    ...
    for pan_cfg in cfg.pans:
        ...
        logger.info(
            ...
            "Generating PAN-level 6A-vs-PR reconciliation per FY.",
...
def _run_one(
    *,
    ...
    try:
        ...
        start_rp, end_rp = rng
        logger.info(
            "[{}/{}] period range {}..{} (PR and 6A sides identical)",
        ...
        # Step 1/4: trigger the match (also fetches the 6A/PR data server-side).
        logger.info("[{}/{}] Step 1/4: trigger 6A-vs-PR match", pan, fy)
